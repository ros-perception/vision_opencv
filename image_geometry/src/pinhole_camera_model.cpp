...
bool updateMat(const MatT & new_mat, MatT & my_mat, cv::Mat_<double> & cv_mat, int rows, int cols)
{
  if (
    (my_mat == new_mat) &&
    (my_mat.size() == static_cast<unsigned int>(cv_mat.rows * cv_mat.cols))) {
    return false;
  }
  my_mat = new_mat;
...
{
  cv::Rect roi = rectifiedRoi();
  return cv::Point2d(uv_reduced.x * binningX() + roi.x, uv_reduced.y * binningY() + roi.y);
}
...
cv::Rect PinholeCameraModel::toFullResolution(const cv::Rect & roi_reduced) const
{
  cv::Rect roi = rectifiedRoi();
  return cv::Rect(
    roi_reduced.x * binningX() + roi.x, roi_reduced.y * binningY() + roi.y,
    roi_reduced.width * binningX(), roi_reduced.height * binningY());
}

cv::Point2d PinholeCameraModel::toReducedResolution(const cv::Point2d & uv_full) const
{
  cv::Rect roi = rectifiedRoi();
  return cv::Point2d((uv_full.x - roi.x) / binningX(), (uv_full.y - roi.y) / binningY());
}
...
cv::Rect PinholeCameraModel::toReducedResolution(const cv::Rect & roi_full) const
{
  cv::Rect roi = rectifiedRoi();
  return cv::Rect(
    (roi_full.x - roi.x) / binningX(), (roi_full.y - roi.y) / binningY(),
    roi_full.width / binningX(), roi_full.height / binningY());
}
...
cv::Rect PinholeCameraModel::rawRoi() const
{
  ...
  return cv::Rect(
    cam_info_.roi.x_offset, cam_info_.roi.y_offset, cam_info_.roi.width, cam_info_.roi.height);
}
...
cv::Point3d PinholeCameraModel::projectPixelTo3dRay(
...
void PinholeCameraModel::rectifyImage(
  const cv::Mat & raw, cv::Mat & rectified, int interpolation) const
{
  assert(initialized());
  ...
  switch (cache_->distortion_state) {
    ...
    case CALIBRATED:
      ...
      if (raw.depth() == CV_32F || raw.depth() == CV_64F) {
        cv::remap(
          raw, rectified, cache_->reduced_map1, cache_->reduced_map2, interpolation,
...
void PinholeCameraModel::unrectifyImage(
  const cv::Mat & rectified, cv::Mat & raw, int interpolation) const
{
  (void)rectified;
  (void)raw;
  (void)interpolation;
  ...
  switch (cache_->distortion_state) {
    ...
    case CALIBRATED:
      ...
      if (rectified.depth() == CV_32F || rectified.depth() == CV_64F) {
        cv::remap(
          rectified, raw, cache_->unrectify_reduced_map1, cache_->unrectify_reduced_map2,
          interpolation, cv::BORDER_CONSTANT, std::numeric_limits<float>::quiet_NaN());
      } else {
        cv::remap(
          rectified, raw, cache_->unrectify_reduced_map1, cache_->unrectify_reduced_map2,
          interpolation);
...
cv::Point2d PinholeCameraModel::rectifyPoint(
  const cv::Point2d & uv_raw, const cv::Matx33d & K, const cv::Matx34d & P) const
{
  ...
  switch (cache_->distortion_model) {
    ...
    default:
      ...
      throw Exception(
        "Wrong distortion model. Supported models: PLUMB_BOB, RATIONAL_POLYNOMIAL and "
        "EQUIDISTANT.");
  }
  return rect32;
}
...
cv::Point2d PinholeCameraModel::unrectifyPoint(
  ...
  switch (cache_->distortion_model) {
    ...
    case EQUIDISTANT:
      cv::fisheye::projectPoints(
        std::vector<cv::Point3d>(1, ray), image_point, r_vec, t_vec, K, D_);
      break;
    default:
      assert(cache_->distortion_model == UNKNOWN_MODEL);
      throw Exception(
...
cv::Rect PinholeCameraModel::rectifyRoi(const cv::Rect & roi_raw) const
{
  ...
  // Since ROI is specified in unbinned coordinates (see REP-104),
  // this has to use K_full_ and P_full_.
  cv::Point2d rect_tl = rectifyPoint(cv::Point2d(roi_raw.x, roi_raw.y), K_full_, P_full_);
  cv::Point2d rect_tr =
    rectifyPoint(cv::Point2d(roi_raw.x + roi_raw.width, roi_raw.y), K_full_, P_full_);
  cv::Point2d rect_br = rectifyPoint(
    cv::Point2d(roi_raw.x + roi_raw.width, roi_raw.y + roi_raw.height), K_full_, P_full_);
  cv::Point2d rect_bl =
    rectifyPoint(cv::Point2d(roi_raw.x, roi_raw.y + roi_raw.height), K_full_, P_full_);

  cv::Point roi_tl(
    static_cast<uint32_t>(std::ceil(std::min(rect_tl.x, rect_bl.x))),
    static_cast<uint32_t>(std::ceil(std::min(rect_tl.y, rect_tr.y))));
  cv::Point roi_br(
    static_cast<uint32_t>(std::floor(std::max(rect_tr.x, rect_br.x))),
...
cv::Rect PinholeCameraModel::unrectifyRoi(const cv::Rect & roi_rect) const
{
  ...
  cv::Point2d raw_br =
    unrectifyPoint(cv::Point2d(roi_rect.x + roi_rect.width, roi_rect.y + roi_rect.height));
  cv::Point2d raw_bl = unrectifyPoint(cv::Point2d(roi_rect.x, roi_rect.y + roi_rect.height));

  cv::Point roi_tl(
    static_cast<uint32_t>(std::floor(std::min(raw_tl.x, raw_bl.x))),
    static_cast<uint32_t>(std::floor(std::min(raw_tl.y, raw_tr.y))));
  cv::Point roi_br(
    static_cast<uint32_t>(std::ceil(std::max(raw_tr.x, raw_br.x))),
    ...
    switch (cache_->distortion_model) {
      case PLUMB_BOB_OR_RATIONAL_POLYNOMIAL:
        ...
        cv::initUndistortRectifyMap(
          K_binned, D_, R_, P_binned, binned_resolution, CV_16SC2, cache_->full_map1,
          cache_->full_map2);
        break;
      case EQUIDISTANT:
...
void PinholeCameraModel::initUnrectificationMaps() const
{
  ...
  if (cache_->unrectify_full_maps_dirty) {
    ...
    // Note: m1type=CV_16SC2 to use fast fixed-point maps (see cv::remap)
    convertMaps(
      float_map_x, float_map_y, cache_->unrectify_full_map1, cache_->unrectify_full_map2, CV_16SC2);
